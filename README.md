...
- **iOS 잠금화면 Live Activity** 에서 현재 리듬을 실시간으로 확인/제어할 수 있게 한다.
...
### 3.1 오늘의 루틴 실행 (iOS 잠금화면 중심)

1. 사용자는 앱에서 하루 리듬을 시작한다.
2. 잠금화면 Live Activity에서 **현재 리듬, 남은 시간, 진행도**를 즉시 확인한다.
3. 잠금화면에서 **일시정지/완료** 액션을 수행한다.
4. 상태 변경은 도메인 상태와 잠금화면 UI에 동시에 반영된다.
5. 세션이 끝나면 다음 리듬으로 자동 전환되거나, 남은 일정 없음 상태를 표시한다.
...
- `CurrentRoutineExecutionCard`
  - 앱 내부 보조 카드
  - 잠금화면과 동일한 핵심 정보(현재 리듬, 남은 시간, 진행률, 다음 단계)를 보여줌
- `RoutineSummaryCard`
  - 루틴 리스트/추천 리스트에서 사용
