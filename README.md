...
잠금 화면처럼 **가볍게 열고**, 플로우에 **깊게 잠기는** 집중 앱 **LockFlow** 의 기획/설계 문서입니다.  
이 문서는 제품 컨셉, 핵심 시나리오, 도메인 모델, 상태관리, 데이터 영속화, 기술 구조를 한 번에 정리합니다.

### iOS: 빌드 실행 시 메인 앱(오늘 탭)부터 열리게 하려면

1. **Xcode 상단 스킴(Scheme)이 `LockFlow` 인지 확인**한다.  
   `LockFlowLiveActivity` 는 **Live Activity 확장** 타깃이라, 이걸로 Run 하면 잠금화면/위젯 쪽 동작만 디버깅하게 되고 **일반 앱 화면이 아니라** 확장 실행 흐름에 가깝다.  
   → 일상 개발·UI 확인은 **`LockFlow` (메인 앱)** 으로 실행한다.

2. 시뮬레이터가 **물리 잠금 화면**만 보이면, 그건 iOS 시뮬레이터 동작이다. 홈으로 가려면 잠금 해제(스와이프)하거나, 시뮬레이터 메뉴에서 기기를 재부팅해 본다.

3. 앱 라우트: 루트 URL(`/`)로 들어오면 **`/(tabs)`(오늘)** 로 보내도록 `app/index.tsx` 에서 처리한다.  
   `lockflow://activity-session?...` 같은 딥링크는 기존처럼 세션 화면으로 연결된다.
...
- 사용자가 **하루 플로우를 정의**하고  
- 그 플로우를 **실제 플로우 실행(RoutineExecution) 단위로 실행**하며  
- **iOS 잠금화면 Live Activity** 에서 현재 플로우를 실시간으로 확인/제어할 수 있게 한다.
- 별도 서버 없이 **로컬(LocalStorage)** 기반으로 빠르게 동작하는 **개인 집중 플로우 도구**를 지향한다.

### 1.2 핵심 아이디어

- **Routine**: 아침, 점심, 저녁, Deep Work 등 시간대/상황별 “템플릿”
- **RoutineTask**: 플로우에 포함된 세부 액션들의 리스트
- **RoutineExecution**: 특정 시간에 실제로 실행되는 플로우 인스턴스(플로우 한 번 실행)
- **Today 대시보드**: 오늘 진행 중/예정된 플로우 실행들을 한눈에 보여주는 홈 화면

LockFlow 는 “해야 할 일을 많이 적는 투두 앱”이 아니라,  
**미리 정의된 플로우를 실행(RoutineExecution)하며 흐름(flow)에 들어가는 앱**이다.
...
### 2.2 플로우 지향 라이프스타일 사용자

- 아침 플로우, 자기 전 플로우 등 **반복되는 패턴**을 만들고 싶은 사람
- 운동, 명상, 독서 등 **습관성 태스크**를 관리하는 데 관심
- 특징
  - 체크리스트보다 **“플로우의 흐름”** 에 집중
...
### 3.1 오늘의 플로우 실행 (iOS 잠금화면 중심)

1. 사용자는 앱에서 하루 플로우를 시작한다.
2. 잠금화면 Live Activity에서 **현재 플로우, 남은 시간, 진행도**를 즉시 확인한다.
3. 잠금화면에서 **일시정지/완료** 액션을 수행한다.
4. 상태 변경은 도메인 상태와 잠금화면 UI에 동시에 반영된다.
5. 세션이 끝나면 다음 플로우로 자동 전환되거나, 남은 일정 없음 상태를 표시한다.

### 3.2 새로운 플로우 만들기 (Routine Setup)

1. 사용자는 “플로우 추가” 버튼을 눌러 새로운 플로우를 만든다.
2. 플로우 이름/카테고리/아이콘/색상을 정한다. (예: Morning Energy)
3. 태스크들을 순서대로 추가한다. (예: 물 마시기 5분, 스트레칭 10분, 계획 세우기 10분)
4. 태스크 순서와 예상 소요 시간으로 **전체 플로우 길이**를 확인한다.
5. 저장 후, 이 플로우를 Today 대시보드에서 플로우 실행으로 실행할 수 있다.

### 3.3 추천 플로우 둘러보기 (Discover)

1. 사용자는 Discover 탭을 열어 **추천 플로우 카드 리스트**를 본다.
2. 상단 필터 칩(All / Morning / Focus / Relax 등)을 눌러 관심 카테고리만 본다.
3. 마음에 드는 카드를 눌러 상세 설명을 확인한 뒤, 내 플로우로 복사/커스터마이징한다.
...
- **RoutineExecution**
  - 실제 실행 중인 플로우 인스턴스(플로우 한 번 실행). 시작/종료, 진행률, 현재 태스크 등
  - 예시 필드: `id`, `routineId`, `startTime`, `endTime`, `progress`, `currentTaskId`
- **Stats**
  - 일별/주별 플로우 완료율, 태스크 수행 기록 등 요약 통계
...
- `Today` 탭 (`src/pages/today`)
  - 오늘의 플로우 실행, 현재 진행 중인 플로우 카드
- `Discover` 탭 (`src/pages/discover`)
  - 추천 플로우 리스트, 카테고리/태그 필터
...
### 5.2 카드 기반 레이아웃 (앱 내부 보조 UI)

- **큰 카드**로 현재 플로우 실행/추천 플로우를 강조
...
- `CurrentRoutineExecutionCard`
  - 앱 내부 보조 카드
  - 잠금화면과 동일한 핵심 정보(현재 플로우, 남은 시간, 진행률, 다음 단계)를 보여줌
- `RoutineSummaryCard`
  - 플로우 리스트/추천 리스트에서 사용
...
- `RoutineExecutionStore`
  - 현재 플로우 실행, 남은 시간, 진행률, 현재 태스크 id
- `RoutineStore`
  - 플로우 목록, 선택된 플로우, 편집 중인 플로우
- `TodayDashboardStore`
...
- 플로우 실행/플로우 기반 **간단한 통계 화면** (주간 완료율, 가장 많이 실행한 플로우 등)
- 플로우 공유/템플릿 갤러리 (초기에는 로컬만, 이후 서버 연동 고려)
- 알림/리마인더와의 연동 (아침 플로우 시작 알림 등)

LockFlow 는 “일상의 **플로우**를 존중하는 잠금 화면 스타일 집중 앱”이라는 방향성을 유지하면서,  
...
**iOS 잠금화면에서 "지금 해야 할 플로우"를 실시간으로 확인하고 제어**하는 데 둔다.
...
1. 잠금화면에 현재 플로우/남은 시간/진행도 노출
2. 잠금화면 일시정지/완료 액션 연동
3. 완료/건너뛰기 시 다음 플로우 자동 반영
4. 코칭 문구/점수/고급 시각효과 확장

### 11.5 구현 진행표 (체크리스트)

- **단일 소스:** [`docs/LIVE_ACTIVITY_STATUS.md`](docs/LIVE_ACTIVITY_STATUS.md)
- Live Activity·네이티브·QA 진행 상황은 위 문서를 갱신한다.
