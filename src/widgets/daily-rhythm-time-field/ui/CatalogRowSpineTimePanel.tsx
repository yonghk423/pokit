...
type Props = {
  ...
  /** 집중 구간 시작. single은 「확인」 시에만 검사 */
  priorityStart?: string;
  /** 집중 구간 종료. single은 「확인」 시에만 검사 */
  priorityEnd?: string;
  disabled?: boolean;
  onScheduleChange: (
    startMinutes: number,
  ...
  /** range: 시작·종료 · single: 알림 등 시각 하나 + 당일/다음 날 */
  scheduleMode?: 'range' | 'single';
  /** 당일/다음 날 질문 (기본: 마무리 시각 문구) */
...
export const CatalogRowSpineTimePanel = forwardRef<CatalogRowSpineTimePanelHandle, Props>(
  ...
  useEffect(() => {
    // 피커를 접는 것만으로 초안을 되돌리지 않는다.
    // 「확인」 후 접으면 expanded가 null이 되는데, 그때 props(옛 시각)로 덮으면 저장이 무시된다.
  ...
  const resolvedDayQuestion =
    dayChoiceQuestion?.trim() || t('dayRhythm.endDateQuestion');
  ...
  const emitSchedule = useCallback(
    (startHhmm: string, endHhmm: string, endsNext: boolean): boolean => {
      ...
      if (isSingle) {
        onScheduleChange(start, start, endsNext);
        return true;
      }
    ...
    [isSingle, onScheduleChange],
  );

  const commitDraft = emitSchedule;

  const confirmSingleNotify = useCallback(
    (hhmm: string, nextDay: boolean): boolean => {
      const start = parseHHmmToMinutes(hhmm);
      if (start === null) return false;
      const window = resolveSpinePriorityWindow(
        priorityStart?.trim() ?? '',
        priorityEnd?.trim() ?? '',
      );
      if (window && !isNotifyTimeWithinPriorityWindow(start, nextDay, window)) {
        Alert.alert(
          t('goalDetail.medicine.notifyOutOfRangeTitle'),
          t('goalDetail.medicine.notifyOutOfRangeMessage'),
        );
        return false;
      }
      if (!commitOnConfirm) {
  ...
  useImperativeHandle(
    ...
    () => ({
      commitPendingSchedule: () => {
        ...
        const parsedEnd = isSingle ? parsedStart : parseHHmmToMinutes(end);
        if (
          parsedStart === null ||
          parsedEnd === null ||
        ...
        onPress={() => {
          ...
          if (commitOnChange) {
            emitSchedule(
              start,
              isSingle ? start : end,
              showEndDateChoice ? nextDay : false,
            );
          }
        }}
      ...
      <View style={[styles.rootSettings, { paddingLeft: contentInsetLeft }]}>
        <SnappedTimePickerField
          ...
          dateCaption={
            showDateLabels
              ? isSingle
                ? draftEndsNext && showEndDateChoice
                  ? resolvedDateLabels.next
                  : resolvedDateLabels.today
                : resolvedDateLabels.start
              : undefined
          ...
          disabled={disabled}
          routineDayStartHhmm={isSingle ? undefined : priorityStart}
          routineDayEndHhmm={isSingle ? undefined : priorityEnd}
          expandedExtra={isSingle ? dayChoiceExtra : undefined}
          onBeforeConfirm={
            isSingle ? (hhmm) => confirmSingleNotify(hhmm, draftEndsNext) : undefined
          }
        />
        {isSingle ? null : (
