...
export function applyHeadingToBlock(
  block: WorkStudyDocBlock,
  level: WorkStudyHeadingLevel | null,
): WorkStudyDocBlock {
  const text = stripHeadingMarks(block.text);
  if (level == null || block.headingLevel === level) {
    const next = { ...block, text, kind: block.kind === 'heading' ? 'paragraph' : block.kind };
  ...
  return {
    ...
    text,
    headingLevel: level,
    kind: block.kind === 'paragraph' ? 'heading' : block.kind,
  };
}
...
function wrapSelection(text: string, start: number, end: number, open: string, close: string, fallbackLine: string): string {
    ...
    .map((block) => {
      switch (block.kind) {
        ...
        case 'table':
          return '[표]';
        case 'image':
          return block.text.trim() ? `[이미지] ${block.text}` : '[이미지]';
        default:
          return block.text;
      }
    })
