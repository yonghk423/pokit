...
const NOTE_TOOLBAR_BG = 'rgba(245, 242, 235, 0.92)';
const NOTE_TOOLBAR_BTN_BG = 'rgba(255, 255, 255, 0.82)';
...
export function StudyNoteCanvas({ document, onChangeDocument, palette }: Props) {
  ...
  const animateList = useCallback(
    (open: boolean) => {
      ...
      Animated.timing(listProgress, {
        ...
        if (finished && !open) setListMounted(false);
      });
    },
    [listProgress],
  );
  ...
  const addPage = useCallback(
    (closeAfter: boolean) => {
      ...
      if (closeAfter) animateList(false);
    },
    [animateList, onChangeDocument],
  );
  ...
  const onToolbarAction = useCallback(
    ...
    [persistDraft, pushUndo, selection.end, selection.start, t],
  );
