...
export function RestSettings({
  ...
  const initial = normalizeRestDetailConfig(dataConfig ?? getInitialRestDataConfig());

  const [restStr, setRestStr] = useState(String(initial.restMin));
  const [elapsedStr, setElapsedStr] = useState(String(initial.elapsedMin));
  const lastRef = useRef<string | null>(null);

  useEffect(() => {
    const payload: RestDetailDataConfig = normalizeRestDetailConfig({
      restMin: parseInt(restStr, 10) || 0,
  ...
  }, [restStr, elapsedStr, onChangeDataConfig]);

  return (
    <View style={[styles.inner, { backgroundColor: c.surfaceLow }]}>
      <ThemedText style={[styles.title, { color: c.onSurface }]}>휴식 타이머</ThemedText>
      <ThemedText style={[styles.sub, { color: c.onVariant }]}>
        총 휴식 시간과 지금까지 쉰 시간을 분 단위로 적어 주세요.
      </ThemedText>
      <View style={styles.row}>
        <View style={styles.col}>
          <ThemedText style={[styles.label, { color: c.onVariant }]}>휴식(분)</ThemedText>
