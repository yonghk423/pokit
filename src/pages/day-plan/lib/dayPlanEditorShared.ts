...
export type PriorityTask = { id: string; title: string };
...
export function getDaySliderPercents(startTime: string, endTime: string) {
  ...
  if (s === null || e === null || e <= s) {
    return { startPct: 18.75, endPct: 47.9, rangeLeftPct: 18.75, rangeWidthPct: 29.15 };
  }
  ...
  return {
    startPct,
    endPct,
    rangeLeftPct: startPct,
    rangeWidthPct: Math.max(endPct - startPct, 0.5),
  };
}
